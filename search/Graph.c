...
Graph newGraph(int maxV) {
	Graph new = malloc(sizeof(struct _GraphRep));
	assert(new != NULL);
	int i, j;
	new->nV = 0;
	new->maxV = maxV;
...
void disposeGraph(Graph g) {
	if (g == NULL) return;
	int i;
	for (i = 0; i < g->nV; i++) free(g->vertex[i]);
	for (i = 0; i < g->maxV; i++) free(g->edges[i]);
	free(g->vertex);
	free(g->edges);
	free(g);
}
...
int addEdge(Graph g, char *src, char *dest) {
	assert(g != NULL);
	if (strEQ(src, dest)) return 1;
	int v = vertexID(src, g, g->nV);
	if (v < 0) {
		if (g->nV >= g->maxV) return 0;
		v = addVertex(src, g->vertex, g->nV);
		g->nV++;
	}
	int w = vertexID(dest, g, g->nV);
	if (w < 0) {
		if (g->nV >= g->maxV) return 0;
...
int findDegree(char *str, Graph g) {
	int degrees = 0;
	int v = vertexID(str, g, g->nV);
	for (int i = 0; i < g->nV; i++) {
		if (g->edges[v][i]) degrees++;
	}
	return degrees;
}
...
int vertexID(char *str, Graph g, int N) {
	int i;
	for (i = 0; i < N; i++)
		if (strEQ(str, g->vertex[i])) return i;
	return -1;
}
...
char *vertex(Graph g, int i) { return g->vertex[i]; }
int edge(Graph g, int y, int x) { return g->edges[y][x]; }
